# this package is based on [krlove/eloquent-model-generator](https://github.com/krlove/eloquent-model-generator) and just for learning !!!
 

# Eloquent Model Generator

Eloquent Model Generator is a tool based on [Code Generator](https://github.com/HarcourtFun/code-generator) for generating Eloquent models.

## Installation
Step 1. Add Eloquent Model Generator to your project:
```
composer require Harcourt/eloquent-model-generator --dev
```
Step 2. Register `GeneratorServiceProvider`:
```php
'providers' => [
    // ...
    Harcourt\EloquentModelGenerator\Provider\GeneratorServiceProvider::class,
];
```
...
Use
```
php artisan Harcourt:generate:model User
```
...
```
php artisan Harcourt:generate:model User --table-name=user
```
...
```
php artisan Harcourt:generate:model User --output-path=/full/path/to/output/directory --namespace=Some\\Other\\NSpace
```
...
```
php artisan Harcourt:generate:model User --base-class-name=Some\\Other\\Base\\Model
```

### backup
Save existing model before generating a new one
```
php artisan Harcourt:generate:model User --backup
```
 ...
 * @property mixed $email  邮件地址
 * @property Role $role  角色
 * @property Article[] $articles 文章
 * @property Comment[] $comments 评价
 */
